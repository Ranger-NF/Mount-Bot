...
export default class ogCommand extends Command {
  ...
  async messageRun(message: Message, args: Args) {
    ...
    var ogMember =
      member1.joinedTimestamp! < member2.joinedTimestamp! ? member1 : member2;
    var nonOg =
      member1.joinedTimestamp! > member2.joinedTimestamp! ? member1 : member2;
    ...
    let ogEmbed = new EmbedBuilder()
      ...
      .setTitle("OG scale")
      .setDescription(
        `${ogMember.user.username} is cooler than ${nonOg.user.username}`,
      )
      .addFields([
        {
          name: `They have a difference of more than ${joinedTimeStamp}!`,
          value: "\u200B",
        },
      ])
      .setImage("attachment://og.png")
      .setFooter({
        text: `Command invoked by ${message.author.tag}`,
